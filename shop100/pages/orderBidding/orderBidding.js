...
Page({
  ...
  onLoad: function (options) {
    ...
    let biddingId = options.biddingId
    let buyNum = options.buyNum
    this.getData({ productId, goodsId, sellerId, biddingId, buyNum })
  },
  ...
  async getData(para) {
    let url = '/order/bidding-' + para.productId + '-' + para.goodsId + '-' + para.sellerId + '-' + para.biddingId + '-' + para.buyNum + '.html'
  ...
  async toPay() {
    ...
    let postData = {
      ...
      actBiddingId: this.data.data.actBidding.id,
      number: this.data.data.number,
      paymentCode: 'ONLINE',
      paymentName: '在线支付',
      addressId: this.data.address.id,
    }
    let data = await request('ordercommitforbidding', postData, true, 'POST')
