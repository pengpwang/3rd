...
Page({
  ...
  getData: async function (id) {
    let url = '/jifen/' + id + '.html'
    let data = await request(url, {}, true, 'GET')
    this.setData({
      data: data.data,
      productStock: data.data.actIntegral.stock
    })
  ...
  addNum() {
    let limitNum = this.data.data.actIntegral.purchase
  ...
  async buyNow() {
    let stock = this.data.data.actIntegral.stock
    let limitNum = this.data.data.actIntegral.purchase
    ...
    wx.navigateTo({
      url: '/pages/orderScore/orderScore?productId=' + this.data.data.product.id + '&goodsId=' + this.data.productGoodsId + '&sellerId=' + this.data.data.seller.id + '&scoreId=' + this.data.data.actIntegral.id + '&buyNum=' + buyNum
    })
  },
})